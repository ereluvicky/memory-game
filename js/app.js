...
function cardOpen() {
  ...
  if (len === 2) {
    if (openedCards[0].type != openedCards[1].type) {
      moveCounter();
      if (openedCards[0].type.slice(0, openedCards[0].type.indexOf("-")) === openedCards[1].type.slice(0, openedCards[1].type.indexOf("-"))) {
        matched();
      } else {
        unmatched();
      }
...
function matched() {
  for (let k in openedCards) {
    openedCards[k].classList.add("match", "disabled");
    openedCards[k].classList.remove("show", "open", "no-event");
  }
  openedCards = [];
}
